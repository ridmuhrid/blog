...
<?php $level=substr(get_the_title(),33);?>
<meta name="keywords" content="game tebak gambar, game tebak gambar level <?php echo $level;?>, kunci jawaban game tebak gambar level <?php echo $level;?>, kunci jawaban game tebak gambar level <?php echo $level;?> terbaru, kunci jawaban tebak gambar, kunci jawaban tebak gambar level <?php echo $level;?>, kunci jawaban tebak gambar level <?php echo $level;?> terbaru, tebak gambar, tebak gambar level <?php echo $level;?>" />
...
<h2 itemprop="headline"><a href="<?php echo esc_url(home_url("/")); ?>" itemprop="url" title="<?php echo esc_attr(get_bloginfo("name","display"));?>" rel="home">MUHRID</a></h2>
</div>
</header>
<main>
...
<?php if(has_post_thumbnail()){$image_data=wp_get_attachment_image_src(get_post_thumbnail_id($post->ID),"thumbnail");$image_url=$image_data[0];$image_width=$image_data[1];$image_height=$image_data[2];
...
<div class="article" itemprop="articleBody"><p><?php echo get_the_title();?> terbaru beserta gambarnya yang bisa membantu Anda saat mengalami kebuntuan dalam bermain game Tebak Gambar. Dapatkan kunci jawaban tebak gambar dengan berbagai level di halaman <a href="https://muhrid.com/tag/kunci-jawaban-tebak-gambar/" title="Kunci Jawaban Tebak Gambar">Kunci Jawaban Tebak Gambar</a>.</p><?php if(get_the_content()==NULL){echo'<p>Mohon maaf, '.get_the_title().' saat ini belum tersedia karena level di dalam game Tebak Gambar belum sampai ke '.str_replace("Kunci Jawaban Tebak Gambar","",get_the_title()).', silakan tunggu atau coba tanyakan soal yang tidak bisa Anda jawab di grup <a href="http://muhrid.com/gruptebakgambar/">Kunci Jawaban Tebak Gambar di WhatsApp</a>, saya akan usahakan membalas pesan Anda sesegera mungkin.</p>';}else{?><div class="center"><?php the_content();?></div><?php }?><div class="ads">
...
</div><p><?php the_title();?> ini terakhir diperbarui pada tanggal <?php echo get_the_modified_date();?> dengan sedikit perubahan. Silakan laporkan melalui grup <a href="http://muhrid.com/gruptebakgambar/">Kunci Jawaban Tebak Gambar di WhatsApp</a> jika ada jawaban yang sudah tidak sesuai dengan soal tebak gambar versi terbaru atau ada soal terbaru yang ingin ditanyakan dan didiskusikan.</p>
<div class="level">Kunci jawaban Tebak Gambar level lainnya:<br>
<?php 
...
while($level->have_posts()){$level->the_post();echo'<a href="'.get_the_permalink().'">'.str_replace("Kunci Jawaban Tebak Gambar ","",get_the_title()).'</a>';}?></div>
</div>
...
<footer>
<p><span>© Muhrid.com 2013-2019. Hak Cipta Dilindungi Undang-Undang</span><span>Dibuat dengan ❤ oleh Muhammad Riduan</span></p>
</footer>
</div>
...
<?php $is_admin=current_user_can('manage_options');if($is_admin){wp_footer();}?>
</body>
</html>